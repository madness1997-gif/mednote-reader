  ...
  return (<><div className={`note-toolbar two-row-toolbar ${notePanel === "text" ? "text-tools-open" : ""}`} role="toolbar" aria-label="Công cụ ghi chú">
            <div className="toolbar-row toolbar-row-primary">
              ...
              <div className="note-sheet-view-control" role="group" aria-label="Cách xem các tờ trong Page">
                <button className={noteSheetViewMode === "single" ? "selected" : ""} onClick={() => setNoteSheetViewMode("single")} aria-pressed={noteSheetViewMode === "single"} aria-label="Từng trang" title="Chỉ hiện tờ đang mở"><Square size={14} /></button>
                <button className={noteSheetViewMode === "continuous" ? "selected" : ""} onClick={() => setNoteSheetViewMode("continuous")} aria-pressed={noteSheetViewMode === "continuous"} aria-label="Liên tục" title="Cuộn tất cả tờ trong Page"><Rows3 size={14} /></button>
              </div>
              ...
              <button className={`paper-button ${notePanel === "paper" ? "active" : ""}`} onClick={() => setNotePanel((panel) => panel === "paper" ? null : "paper")} aria-expanded={notePanel === "paper"} aria-label="Giấy" title="Giấy"><NotebookTabs size={17} /><ChevronDown size={11} /></button>
              {!layout.showNoteSidebar && <button className="note-sidebar-show-button" onClick={() => layout.setNoteSidebarVisibility(true)} aria-label="Hiện thanh điều hướng Note" title="Hiện thanh điều hướng Note"><PanelRightOpen size={16} /></button>}
            </div>
            <div className="toolbar-row toolbar-row-tools">
              ...
              <div className={`toolbar-cluster object-layer-cluster ${selectedExcerpt ? "has-selection" : ""}`} aria-label="Sắp xếp lớp đối tượng">
                <span className="layer-control-label" title={selectedExcerpt ? "Đối tượng đang chọn" : "Chọn một khung chữ hoặc ảnh để sắp xếp lớp"}><Layers2 size={16} /></span>
            ...
            {notePanel === "text" && <>
              ...
              <div className="toolbar-scroll-shell">
                ...
                <div ref={textParagraphToolbarRef} className="toolbar-row text-command-row text-paragraph-row" onWheel={scrollTextToolbarWithWheel} aria-label="Định dạng đoạn, ký hiệu và bảng">
                  ...
                  <span className="toolbar-mini-divider" />
                  <button className={`word-command-button ${textInsertPopover === "symbols" ? "selected" : ""}`} onPointerDown={(event) => event.preventDefault()} onClick={(event) => openTextPopover("symbols", event.currentTarget)} title="Chèn ký hiệu" aria-label="Ký hiệu"><Omega size={16} /></button>
                  <button className={`word-command-button sticker-menu-trigger ${textInsertPopover === "stickers" ? "selected" : ""}`} onPointerDown={(event) => event.preventDefault()} onClick={(event) => openTextPopover("stickers", event.currentTarget)} title="Chèn sticker note" aria-label="Mở thư viện sticker note" aria-expanded={textInsertPopover === "stickers"}><MessageSquareText size={16} /></button>
                  <button className={`word-command-button ${textInsertPopover === "equation" ? "selected" : ""}`} onPointerDown={(event) => event.preventDefault()} onClick={(event) => openTextPopover("equation", event.currentTarget)} title="Chèn công thức" aria-label="Công thức"><Sigma size={16} /></button>
                  <button className={`word-command-button ${textInsertPopover === "table" ? "selected" : ""}`} onPointerDown={(event) => event.preventDefault()} onClick={(event) => openTextPopover("table", event.currentTarget)} title="Chèn bảng" aria-label="Bảng"><Table2 size={16} /></button>
