            ...
            {visiblePages.map((page) => {
              ...
              return <article className={`note-sidebar-page ${page.active ? "active" : ""}`} key={page.id}>
                ...
                {expanded && <div className="note-sidebar-sheets">{page.sheets.map((sheet) => <div className={`note-sidebar-sheet ${sheet.active ? "active" : ""}`} key={sheet.id}>
                  ...
                  <span className="note-sidebar-row-actions"><button type="button" onClick={() => run(() => controller.renameSheet(sheet))} title="Đổi tên sheet" aria-label={`Đổi tên sheet ${sheet.label}`} disabled={busy}><Pencil size={11} /></button><button type="button" onClick={() => run(() => controller.transferSheet(sheet))} title="Di chuyển sang Page khác" aria-label={`Di chuyển sheet ${sheet.label}`} disabled={busy}><MoveRight size={11} /></button><button type="button" onClick={() => run(() => controller.moveSheet(sheet, sheet.order - 1))} disabled={busy || sheet.order === 0} title="Đưa lên" aria-label="Đưa tờ lên">↑</button><button type="button" onClick={() => run(() => controller.moveSheet(sheet, sheet.order + 1))} disabled={busy || sheet.order === page.sheets.length - 1} title="Đưa xuống" aria-label="Đưa tờ xuống">↓</button><button type="button" onClick={() => run(() => controller.deleteSheet(page, sheet))} title="Xóa tờ" aria-label={`Xóa tờ ${sheet.order + 1}`} disabled={busy}><Trash2 size={11} /></button></span>
                </div>)}</div>}
              </article>;
            })}
