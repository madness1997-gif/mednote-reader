  ...
  return {
    ...
    transform(code, id) {
      ...
      next = replaceRequired(
        next,
        NOTES_TO_NAV,
        '        </section>\n        )}\n\n        {workspaceMode !== "reader" && showNoteSidebar && <aside className="note-navigation-host" aria-label="Điều hướng ghi chú"><NoteSidebar onRequestClose={() => setNoteSidebarVisibility(false)} /></aside>}',
