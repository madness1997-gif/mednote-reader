...
test('legacy logical pages migrate into v6 Page, Sheet, and SheetContent records', async ({ page }) => {
  ...
  await expect.poll(async () => page.evaluate(async () => {
    ...
    const result = {
      meta: await read('library:v6:meta'),
      notebook: await read('library:v6:notebook:schema-notebook-1'),
      section: await read('library:v6:section:schema-section-1'),
      page: await read('library:v6:page:schema-page-1'),
      sheet1: await read('library:v6:sheet:schema-sheet-1'),
      sheet2: await read('library:v6:sheet:schema-sheet-2'),
      content1: await read('library:v6:sheet-content:schema-sheet-1'),
      content2: await read('library:v6:sheet-content:schema-sheet-2'),
    ...
    sheet1: { id: 'schema-sheet-1', pageId: 'schema-page-1', order: 0 },
    sheet2: { id: 'schema-sheet-2', pageId: 'schema-page-1', order: 1 },
    content1: { body: 'Nội dung sheet 1' },
    content2: { body: 'Nội dung sheet 2' },
    legacyMeta: null,
  });
  ...
  const records = await page.evaluate(async () => {
    ...
    const result = {
      meta: await read('library:v6:meta'),
      sheet1: await read('library:v6:sheet-content:schema-sheet-1'),
      sheet2: await read('library:v6:sheet-content:schema-sheet-2'),
    };
    db.close();
    return result;
  });
  ...
  for (const sheet of [normalized.sheet1, normalized.sheet2]) {
    expect(sheet).not.toHaveProperty('logicalPageId');
    expect(sheet).not.toHaveProperty('logicalPageTitle');
    expect(sheet).not.toHaveProperty('title');
    expect(sheet).not.toHaveProperty('sheetOrder');
  }
  ...
  const link = await page.evaluate(async (key) => {
    ...
    db.close();
    return result;
  }, `library:v6:note-document-link:${meta.linkIds[0]}`);
