...
test("page composes one document workspace controller and document UI consumes that boundary", async () => {
  const [page, controller, context, topBar, library, stage, toolbar] = await Promise.all([
  ...
  assert.match(context, /documents: DocumentWorkspaceController/);
  assert.match(stage, /useReaderPaneControllers/);
  assert.match(toolbar, /useReaderPaneControllers/);
  for (const consumer of [topBar, library]) {
