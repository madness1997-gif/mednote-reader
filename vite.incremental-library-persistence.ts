...
const SYNC_START_REPLACEMENT = `    if (!silent) setToast("Đang lưu toàn bộ dữ liệu lên Google Drive…");\n    try {\n      const syncSnapshot = await materializeIncrementalLibrary({ workspaces, activeWorkspaceId, readerShare, workspaceMode, noteZoom, savedAt: localSavedAtRef.current || Date.now() } as any) as PersistedLibrary;\n      const syncWorkspaces = syncSnapshot.workspaces;\n      const remoteFiles = await listDriveAppFiles(token);`;
